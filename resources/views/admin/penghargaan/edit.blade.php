    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg"> 
                ...
                <form method="POST" action="{{ route('admin.penghargaan.update', $penghargaan) }}" enctype="multipart/form-data"> 
                    @csrf
                    @method('PUT')
                    <div>
                        <x-input-label for="name" :value="__('Nama Penghargaan')" />
                        <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" 
                          value="{{ $penghargaan->name }}" required autofocus autocomplete="name" />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="lokasi_tahun" :value="__('Lokasi & Tahun (cth: Bali, 2020)')" />
                        <x-text-input id="lokasi_tahun" class="block mt-1 w-full" type="text" name="lokasi_tahun" 
                          value="{{ $penghargaan->lokasi_tahun }}" required autocomplete="lokasi_tahun" />
